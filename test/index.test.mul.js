...
test(`stopInstancePubSubMul: should accept JSON-formatted event payload`, async t => {
  const mocks = getMocks();
  const sample = getSample();
  const pubsubData = { "zone": "test-zone", "instances": ["k-report", "k-cs-portal"] };
  mocks.event.data.data = Buffer.from(JSON.stringify(pubsubData)).toString(
    'base64'
  );
...
test(`stopInstancePubSubMul: should fail with missing 'zone' attribute`, async t => {
  const mocks = getMocks();
  const sample = getSample();
  const pubsubData = { instances: ['test-instance'] };
  mocks.event.data.data = Buffer.from(JSON.stringify(pubsubData)).toString(
    'base64'
  );
  sample.program.stopInstancePubSub(mocks.event, mocks.callback);
